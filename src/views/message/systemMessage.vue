<template>
	<div class="pb-20 MessageAppoint2">
		...
		<div class="bg-f mt-5 pb-50 pt-30 pl-20">
			<el-form :model="form" :rules="rules" ref="form" label-width="85px">
				...
				<el-form-item label="消息类型：" prop="receiveType">
					<el-radio-group v-model.trim="form.receiveType">
						<el-radio :label="1">公告通知</el-radio>
						<el-radio :label="3">系统更新</el-radio>
					</el-radio-group>
				</el-form-item>
				<el-form-item label="消息内容：" prop="MOdxnr">
		...
		return {
			...
			rules: {
				...
				MOdxnr: [
					...
					{max: 4000,message:'内容不能超过4000字符',trigger : 'change'}
				],
				MOtype: [
	...
	methods: {
		...
		submit() {
			...
			this.$refs.form.validate((valid) => {
				if (valid) {
					this.loading = true;
					let sendUrl = '/foundation/message/massTimingSend';
					let sendObj = {
						receiveType : this.form.receiveType,
						messageContent : this.form.MOdxnr,
						businessTypeList : [2],
					};
					if(this.form.MOkhxz === 2){
						sendObj.loginNameListStr = this.form.MOkhText;
					}
					...
					axiosApi(sendUrl,sendObj,(res) => {
						...
						this.errorMessage = data.data['invaliLoginNameList-2'].length > 0 ? 
											data.data['invaliLoginNameList-2'].join(' , ') : '';
						this.successMessage = data.data['validLoginNameList-2'].length > 0 ? 
											data.data['validLoginNameList-2'].join(' , ') : ''; */
